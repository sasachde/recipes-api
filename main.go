...
func init() {
	/*
		...
		insertFromMany, err := collection.InsertMany(ctx, listOfRecipes)
		if err != nil {
			log.Fatal(err)
		}
		log.Println("Inserted Recipes: ", len(insertFromMany.InsertedIDs))
	*/
	ctx = context.Background()
...
func NewRecipeHandler(c *gin.Context) {
	...
	if err != nil {
		fmt.Println(err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Error while inserting new record"})
		return
	}
